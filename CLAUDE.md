...
`terraform.tfvars`, `backend.hcl`, and `*.tfstate*` are all gitignored (`.gitignore`) — what you see committed is only the `.example` files. Locally-present `terraform.tfvars`/`backend.hcl`/`terraform.tfstate*` files in this working tree are leftover local-run artifacts, not tracked state; don't treat their contents (currently: bucket `tfstate-pfe-2026-495220`, `node_count=3`/`max_node_count=5`/`node_vm_size=e2-standard-2` in `environments/staging/terraform.tfvars` — these drift over time and have changed at least once already, e.g. `node_vm_size` was previously `e2-standard-4`) as necessarily the canonical/current values — cross-check against GitHub vars if it matters.
...
- **Locally-present `terraform.tfvars`/`backend.hcl`/`terraform.tfstate*` files are gitignored, not canonical** — their values (currently: bucket `tfstate-pfe-2026-495220`, `node_count=3`/`max_node_count=5`/`node_vm_size=e2-standard-2`) may not match the GitHub `vars.*` actually driving CI, and have already drifted once (`node_vm_size` used to be `e2-standard-4`); check GitHub repo variables for ground truth on live values.
- **`README.md` is stale in several places, not just `moved.tf`** — it references an `environments/staging/moved.tf` ("blocs `moved` — migration state IAM") that does not exist in the current tree (don't go looking for it); its "Structure" section still shows only 4 modules (missing `modules/cnpg_backup` and the second `artifact_registry` instantiation); and its workflow job table/triggers describe a daily 13:30 UTC schedule plus `detect-drift` and `unlock` jobs that no longer exist or run (see Workflow section above — schedule commented out in `2bb1607`, `detect-drift` deleted in `fa8b32c`, `unlock` commented out in `5413dea`/`db7d786`). Treat this repo's `CLAUDE.md`, not `README.md`, as the source of truth for current workflow/module structure.
